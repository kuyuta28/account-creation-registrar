...
def _default_probe_ms() -> int:
    """Lazily load probe timeout from config."""
    from ..config.settings import load_config
    return load_config().timeouts.probe_timeout_ms


def _default_popup_close_ms() -> int:
    """Lazily load popup close probe timeout from config."""
    from ..config.settings import load_config
    return load_config().timeouts.popup_close_probe_ms
...
async def try_locator(
    ...
    *,
    ...
    """
    _timeout = timeout_ms if timeout_ms is not None else _default_probe_ms()
    try:
        await locator.first.wait_for(state=state, timeout=_timeout)
        return True
...
async def first_visible(
    ...
    *,
    ...
    for sel in selectors:
        ...
        if await try_locator(loc, timeout_ms=_timeout):
            return loc
    return None
...
async def click_first_visible(
    ...
    *,
    ...
    """
    _timeout = timeout_ms if timeout_ms is not None else _default_probe_ms()
    loc = await first_visible(page, selectors, timeout_ms=_timeout)
    if loc is None:
        return False
    await loc.click()
    return True
...
async def try_close_popup(page: Page, *, timeout_ms: int | None = None) -> None:
    """
    ...
    """
    _timeout = timeout_ms if timeout_ms is not None else _default_popup_close_ms()
    close_selectors = [
    ...
    await click_first_visible(page, close_selectors, timeout_ms=_timeout)
