"""services/klingai_com/flow.py - KlingAI login automation recipe.

Pure browser automation — nhận `page`/`ctx` từ gateway task (headless=False,
visible cho user login Google thủ công). Entrypoint `_login_flow` mở login page,
đợi user login, extract email. `InternalClient` persist session_state vào DB.
"""
...
async def _login_flow(
    ...
    """Pure automation: mở login page, đợi user login Google, extract email.

    Browser/context do caller (gateway) mở với headless=False. Trả về AccountRecord
    hoặc None nếu hết timeout. KHÔNG save DB — caller save.
    """
    kling = cfg.klingai
    ...
    return AccountRecord(service="KLINGAI", email=email, password="", api_key="")
