"""services/leonardo_ai/flow.py - Leonardo AI signup automation recipe.

Pure browser automation — nhận `page` từ gateway task, không mở browser.
Entrypoint `_run` chạy state-machine signup (Leonardo dùng multi-step auth flow
không cố định thứ tự) đến khi reach dashboard.
"""
...
async def _click_turnstile(page: Page, log_fn: LogFn) -> None:
    ...
    except Exception as exc:  # noqa: BLE001 - best-effort UI action
...
async def _run(
    page: Page, mailbox: Mailbox, email: str, password: str, cfg: AppConfig, log_fn: LogFn,
) -> AccountRecord | None:
    leo = cfg.leonardo
    t = cfg.timeouts
