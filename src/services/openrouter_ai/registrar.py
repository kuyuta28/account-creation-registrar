"""registrar.py - OpenRouter account registration via camoufox."""
...
from ...core.page_utils import dump_debug_html as _dump_debug_html
from ...core.password import generate_password
from ...core.storage import AccountRecord
from ...mail.client import Mailbox, create_mailbox, extract_link, get_message_body, wait_for_message
...
async def _wait_and_submit(page: Any, log_fn: LogFn, debug_dir: Path) -> None:
    ...
    # Clerk co the da tu submit -> check truoc
    ...
    try:
        # Race: button enabled HOAC page chuyen sang verify
    ...
    # Neu da o verify page thi return
    if await _already_on_verify(page):
        log_fn("  Verify page reached (Clerk auto-submitted)")
        return

    # Button enabled nhung chua submit -> click
    log_fn("  Button enabled - clicking")
    await _click_primary_button(page)

    # Check ngay lập tức sau click — Clerk có thể trả lỗi inline
    await page.wait_for_timeout(2_000)
...
async def _enable_privacy_options(page: Any, timeout_ms: int, log_fn: LogFn, debug_dir: Path) -> None:
    ...
    for i in range(count):
        ...
        if state != "checked":
            ...
            await toggle.click()
            await page.wait_for_timeout(1500)  # Chờ API call save hoàn tất
        else:
...
async def register_openrouter(
    ...
    """Đăng ký OpenRouter bằng camoufox."""
    mailbox  = await create_mailbox(cfg.mail.providers_for("openrouter"), log_fn=log_fn)
    email    = mailbox.email
    ...
    async with open_browser(cfg) as browser:
        ...
        record = await _signup_flow(page, browser, mailbox, email, password, cfg, log_fn)
